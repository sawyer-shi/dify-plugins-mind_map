...
### 版本历史
- **v0.0.10** (2026-09-18):
  - 移植市场版验证过的改进：长文本节点 CJK 感知自动换行（英文单词和连续数字不拆断）、渲染串行化与 gevent 感知线程池执行（`RENDER_GATE` / `run_heavy`）、大文件传输门锁（`SEND_GATE`）、8300px 像素预算自动降 DPI、绕过 pyplot 全局状态创建画布
  - 移植市场版视觉风格：双层节点（彩色外环 + 主题内填充 + 明暗自适应对比文字色）、随深度递减的粗边框（根节点 20px 递减到 3px）与粗连接线、浅层节点大圆角；已适配全部四个主题
  - 所有工具新增主题选择：`经典白`、`暗夜黑`、`护眼黄`（暖色纸感）、`工业灰`（暗色画布小格子背景，默认主题）；`经典白` 与原版外观一致
  - 增强守护进程兼容性：matplotlib ≥ 3.11 时启用官方 `MPL_IGNORE_SYSTEM_FONTS` 开关完全跳过系统字体扫描，老版本回退到 subprocess 隔离方案
  - 插件启动时增加渲染器预热（matplotlib/numpy/PIL 导入 + 微型渲染 + 字体探测），避免首次请求超时
  - manifest 补充 `privacy`、`repo`、`minimum_dify_version` 字段；`dify_plugin` 依赖升级到 >= 0.9.0
